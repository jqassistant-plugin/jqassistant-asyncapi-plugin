...
class BindingsIT extends AbstractPluginIT {
    ...
    void messageBindings() {
        ...
        Query.Result<Query.Result.CompositeRowObject> messageBindings = store.executeQuery(
                "MATCH (:Message {referenceableKey:'userSignupMessage'})-[DEFINES_BINDING]->(bindings:MessageBinding) return bindings");
        assertThat(messageBindings.hasResult()).isTrue();

        Query.Result<Query.Result.CompositeRowObject> jmsBinding = store.executeQuery(
                "MATCH (:Message {referenceableKey:'userSignupMessage'})-[DEFINES_BINDING]->(:MessageBinding)-[DEFINES_JMS_BINDING]->(jms:JmsMessage) return jms");
        assertThat(jmsBinding.hasResult()).isTrue();
        ...
        Query.Result<Query.Result.CompositeRowObject> kafkaBinding = store.executeQuery(
                "MATCH (:Message {referenceableKey:'userSignupMessage'})-[DEFINES_BINDING]->(:MessageBinding)-[DEFINES_KAFKA_BINDING]->(kafka:KafkaMessage) return kafka");
        assertThat(kafkaBinding.hasResult()).isTrue();
    ...
    void serverBindings() {
        ...
        Query.Result<Query.Result.CompositeRowObject> serverBindings = store.executeQuery(
                "MATCH (:Server {referenceableKey:'oneServer'})-[DEFINES_BINDING]->(bindings:ServerBinding) return bindings");
        assertThat(serverBindings.hasResult()).isTrue();

        Query.Result<Query.Result.CompositeRowObject> jmsBinding = store.executeQuery(
                "MATCH (:Server {referenceableKey:'oneServer'})-[DEFINES_BINDING]->(:ServerBinding)-[DEFINES_JMS_BINDING]->(jms:JmsServer) return jms");
        assertThat(jmsBinding.hasResult()).isTrue();
        ...
        Query.Result<Query.Result.CompositeRowObject> kafkaBinding = store.executeQuery(
                "MATCH (:Server {referenceableKey:'oneServer'})-[DEFINES_BINDING]->(:ServerBinding)-[DEFINES_KAFKA_BINDING]->(kafka:KafkaServer) return kafka");
        assertThat(kafkaBinding.hasResult()).isTrue();
    ...
    void operationBindings() {
        ...
        Query.Result<Query.Result.CompositeRowObject> operationBindings = store.executeQuery(
                "MATCH (:Operation {referenceableKey:'oneOperation'})-[DEFINES_BINDING]->(bindings:OperationBinding) return bindings");
        assertThat(operationBindings.hasResult()).isTrue();

        Query.Result<Query.Result.CompositeRowObject> kafkaBinding = store.executeQuery(
                "MATCH (:Operation {referenceableKey:'oneOperation'})-[DEFINES_BINDING]->(:OperationBinding)-[DEFINES_KAFKA_BINDING]->(kafka:KafkaOperation) return kafka");
        assertThat(kafkaBinding.hasResult()).isTrue();
