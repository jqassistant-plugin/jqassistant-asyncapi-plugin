...
class OperationsIT extends AbstractPluginIT {
    ...
    public void bindings() {
        //operation binding
        Query.Result<Query.Result.CompositeRowObject> bindings = store.executeQuery("MATCH (:AsyncAPI:Contract)-[:DEFINES_OPERATION]->(:Operation {referenceableKey:'sendProcess'})-[:SUPPORTS_BINDINGS]->(bindings:OperationBindings) return bindings");
        ...
        Query.Result<Query.Result.CompositeRowObject> kafka = store.executeQuery("MATCH (:AsyncAPI:Contract)-[:DEFINES_OPERATION]->(:Operation {referenceableKey:'sendProcess'})-[:SUPPORTS_BINDINGS]->(:OperationBindings)-[:DEFINES_KAFKA_BINDING]->(kafka:KafkaOperation) return kafka");
